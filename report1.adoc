...
|====================
| Protocol| Communication Types synchronous, asynchronous and continuous push from server| Speed with large payload |Connection Break Recognition | Web Compatibility
...
| **MQTT (Moquette)** https://github.com/andsel/moquette | Synchronous communication not possible | Fast | Handled by the protocol | Libraries available eg: Eclipse Paho JavaScript Client
