...
report has the topology as shown in the   figure below. The server side has services
which could be accessed by the client side delegates through an application
layer protocol which also handles all the data exchange activities between the two
...
This report deals with evaluating few of the popular application
...
1) Suitability of the Application layer protocol to
the Server/Client architecture under discussion.

2) The transport layer tests like reliability,
bandwidth and the frequency of data exchange between the server and the client.

3) The speed of data transfer for payload ranging
within 200kB.

4) Tests on LAN and wireless LAN networks.

5) Ability to detect unexpected connection break.

**Test Setup:**

The test setup consists of an OSGi based server and client
applications which work with different application layer protocol APIs like
CoAP, MQTT and Websocket. The application layer protocols can be tested by
simply plugging in the implementation bundle with the server and the client
application. The server application is run on the Raspberry Pi board and the
...
client has a simple text editor where the test payload is entered and a URI
...
each frame an acknowledgement is expected and the next frame is only sent after
...
With **UDP** there is
no connection that is established between the server and the client. Each datagram
...
ping-pong mechanism (connection health polling) needs to be built to determine whether
the peer is still available. Unfortunately this is not built into CoAP and the
application needs to take care of this.

With **TCP** unlike
UDP there is a connection that is established between the server and the
